...
#define CTEST_IMPL_STRUCT(sname, tname, tskip, tdata, tsetup, tteardown)       \
    static struct ctest CTEST_IMPL_TNAME(sname, tname)                         \
        CTEST_IMPL_SECTION = {.ssname = #sname,                                \
                              .ttname = #tname,                                \
...
#define CTEST_IMPL_CTEST2(sname, tname, tskip)                                 \
    ...
    static void CTEST_IMPL_FNAME(sname, tname)(                                \
        struct CTEST_IMPL_DATA_SNAME(sname) * data);                           \
    CTEST_IMPL_STRUCT(                                                         \
        sname, tname, tskip, &CTEST_IMPL_DATA_TNAME(sname, tname),             \
        &CTEST_IMPL_SETUP_FPNAME(sname), &CTEST_IMPL_TEARDOWN_FPNAME(sname));  \
    static void CTEST_IMPL_FNAME(sname, tname)(                                \
        struct CTEST_IMPL_DATA_SNAME(sname) * data)
...
void assert_str(const char *exp, const char *real, const char *caller,
                int line);
#define ASSERT_STR(exp, real) assert_str(exp, real, __FILE__, __LINE__)

void assert_data(const unsigned char *exp, size_t expsize,
                 const unsigned char *real, size_t realsize, const char *caller,
                 int line);
...
void assert_equal_u(uintmax_t exp, uintmax_t real, const char *caller,
                    int line);
#define ASSERT_EQUAL_U(exp, real) assert_equal_u(exp, real, __FILE__, __LINE__)

void assert_not_equal(intmax_t exp, intmax_t real, const char *caller,
                      int line);
#define ASSERT_NOT_EQUAL(exp, real)                                            \
    assert_not_equal(exp, real, __FILE__, __LINE__)

void assert_not_equal_u(uintmax_t exp, uintmax_t real, const char *caller,
                        int line);
...
void assert_dbl_near(double exp, double real, double tol, const char *caller,
                     int line);
...
void assert_dbl_far(double exp, double real, double tol, const char *caller,
                    int line);
...
void assert_data(const unsigned char *exp, size_t expsize,
    ...
    size_t i;
    if (expsize != realsize) {
        CTEST_ERR("%s:%d  expected %" PRIuMAX " bytes, got %" PRIuMAX, caller,
                  line, (uintmax_t)expsize, (uintmax_t)realsize);
    }
    for (i = 0; i < expsize; i++) {
        if (exp[i] != real[i]) {
...
void assert_equal(intmax_t exp, intmax_t real, const char *caller, int line) {
    if (exp != real) {
        CTEST_ERR("%s:%d  expected %" PRIdMAX ", got %" PRIdMAX, caller, line,
                  exp, real);
    }
}
...
void assert_equal_u(uintmax_t exp, uintmax_t real, const char *caller,
                    int line) {
    if (exp != real) {
        CTEST_ERR("%s:%d  expected %" PRIuMAX ", got %" PRIuMAX, caller, line,
                  exp, real);
    }
}
